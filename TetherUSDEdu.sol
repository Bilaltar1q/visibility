...
// File contracts/StreamXToken.sol
...
contract TetherUSDEdu is ERC20, Ownable {
    ...
    /// @notice Check if daily counter needs to be reset
    function _checkAndResetDailyCounter() internal {
        // Reset if 24 hours have passed since last reset
        if (block.timestamp >= lastResetTimestamp + 1 days) {
            dailyApprovalCount = 0;
            lastResetTimestamp = block.timestamp;
            emit DailyCounterReset(block.timestamp);
        }
    }

    /// @notice Get remaining approval allowance for today
    function getDailyApprovalRemaining() public view returns (uint256) {
        // If 24 hours have passed, the full limit is available
        if (block.timestamp >= lastResetTimestamp + 1 days) {
            return DAILY_APPROVAL_LIMIT;
        }

        if (dailyApprovalCount >= DAILY_APPROVAL_LIMIT) {
            return 0;
        }

        return DAILY_APPROVAL_LIMIT - dailyApprovalCount;
    }

    /// @notice Get current daily approval usage
    function getDailyApprovalUsage() public view returns (uint256) {
        // If 24 hours have passed, usage is reset to 0
        if (block.timestamp >= lastResetTimestamp + 1 days) {
            return 0;
        }

        return dailyApprovalCount;
    }

    /// @notice Owner can manually reset the daily counter (emergency function)
